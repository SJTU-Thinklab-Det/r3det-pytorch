...
from r3det.core import obb2poly_np, poly2obb_np
from r3det.ops import obb_nms, poly_nms, polygon_iou, rnms
...
class DOTADataset(CustomDataset):
    ...
    Args:
        ...
        difficulty (bool, optional): The difficulty threshold.
    """
    ...
    def __init__(self,
        ...
        self.merge_nms = 'poly' if version == 'v2' else 'obb'
...
def eval_map(det_results,
    ...
    for i in range(num_classes):
        ...
        cls_dets, cls_gts, cls_gts_ignore = get_cls_results(
            det_results, annotations, i, version)
...
def get_cls_results(det_results, annotations, class_id, version):
    ...
    cls_dets = [
        np.zeros((0, 9), dtype=np.float64) if img_res[class_id].size == 0 else
        obb2poly_np(img_res[class_id], version) for img_res in det_results
    ]
...
def _merge_func(info, CLASSES, iou_thr, version, merge_nms):
    """Merging patch bboxes into full image.

    Params:
        CLASSES (list): Label category.
        iou_thr (float): Threshold of IoU.
        version (str): Angle representations. Defaults to 'v1'.
        merge_nms (str): Type of nms.
    """
    ...
    if merge_nms == 'poly':
        ...
        for i in range(len(CLASSES)):
            ...
            if len(dets[labels == i]) == 0:
                ...
                cls_dets = obb2poly_np(dets[labels == i], version)
                _, keep = poly_nms(cls_dets, iou_thr, device_id=0)
                big_img_results.append(dets[labels == i][keep])
    ...
    for i in range(len(CLASSES)):
        ...
        nms_dets, _ = nms_op(cls_dets, iou_thr)
        big_img_results.append(nms_dets)
    return img_id, big_img_results


def polygon_overlaps(polygons1, polygons2):
    """Calculate overlap between two set of polygons.

    Args:
        polygons1 (torch.Tensor): shape (m, 8).
        polygons2 (torch.Tensor): shape (n, 8).
    Returns:
        narray: shape (m, n).
    """
    p1 = torch.tensor(polygons1[:, :8], dtype=torch.float64)
    p2 = torch.tensor(polygons2[:, :8], dtype=torch.float64)
