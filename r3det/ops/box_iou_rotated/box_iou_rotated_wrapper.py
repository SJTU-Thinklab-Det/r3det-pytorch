...
def obb_overlaps(bboxes1,
                 bboxes2,
                 mode='iou',
                 is_aligned=False,
                 device_id=None):
    """Compute overlaps of polygons.

    Args:
        bboxes1 (torch.Tensor): shape (m, 5)
        bboxes2 (torch.Tensor): shape (n, 5)
        mode (str, optional): iou mode such as 'iou' | 'iof'.
    ...
    if bboxes1_th.numel() == 0 or bboxes2_th.numel() == 0:
        ...
        outputs = aligned_obb_overlaps(bboxes1_th, bboxes2_th, mode)
    else:
        outputs = box_iou_rotated_ext.overlaps(bboxes1_th, bboxes2_th,
                                               mode == 'iou')
...
def aligned_obb_overlaps(bboxes1, bboxes2, mode='iou'):
    ...
    inter_pts, inter_masks = poly_intersection(bboxes1_pts, bboxes2_pts,
                                               areas1, areas2)
...
def obb2poly(obboxes):
    """Convert obboxes to polygons.

    Args:
        obboxes (torch.Tensor): Oriented bboxes.
    Returns:
...
def shoelace(pts):
    ...
    xyxy = pts[..., 0] * roll_pts[..., 1] - roll_pts[..., 0] * pts[..., 1]
    areas = 0.5 * torch.abs(xyxy.sum(dim=-1))
    return areas


def convex_areas(pts, masks):
    """Calculate the convex areas of polygons according to pts and masks.
...
def poly_intersection(pts1, pts2, areas1=None, areas2=None, eps=1e-6):
    """Calculate the intersection points and the mask of whether points is
    inside the lines.
    Reference:
    ...
    Returns:
        all_pts (torch.Tensor): The intersection points.
        masks (torch.Tensor): The mask of whether points is inside the lines.
    """
    ...
    x1, y1, x2, y2 = lines1.unbind(dim=-1)
    x3, y3, x4, y4 = lines2.unbind(dim=-1)
